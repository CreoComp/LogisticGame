...
public class RouteStatisticsService
{
    ...
    public float GetDistance()
    {
        ...
        {
            distance += Vector2.Distance(edge.FirstPoint.GetPosition(), edge.SecondPoint.GetPosition()) * Constants.MapScale * 10;
        }
        return distance;
    }
    ...
    public float GetFuelCost() =>
        ((routeInfo.car.fuelConsumption / 100f) * GetDistance()) * Constants.FuelCost;

    public TimeSpan GetTimeRoute()
    {
        TimeSpan time = TimeSpan.FromHours(GetDistance() / 40f);

        foreach(var station in routeInfo.conductedStations)
        {
            time += TimeSpan.FromMinutes(Constants.TimeAtTheStation);
        }
        return time;
    }
